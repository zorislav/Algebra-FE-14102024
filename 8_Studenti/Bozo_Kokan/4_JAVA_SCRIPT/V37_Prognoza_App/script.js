...
async function main() {
  ...
  async function buttonElOnClick() {
    ...
    if (!cityName) {
      ...
      if (coordinates) {
        inputLocation.innerHTML = coordinates.city;
  ...
  function showWeatherData(data) {
    ...
    timezone.innerHTML = tzMap.get(tzOff);
    ...
    currentWeatherItemsEl.innerHTML = `
          ...
          <div class="weather-item">
              <div>Sunrise</div>
              <div>${window.moment(sunrise * 1000).utcOffset(tzOff / 60).format("HH:mm a")}</div>
          </div>
          <div class="weather-item">
              <div>Sunset</div>
              <div>${window.moment(sunset * 1000).utcOffset(tzOff / 60).format("HH:mm a")}</div>
          </div>
      `;
